...
from .utils.paths import resolve_path, get_project_root
...
class Network_2D():
    def __init__(self, config):
        ...
        cfg = Config.fromfile(resolve_path(config["network2d"]["config_path"]))
        
        # Resolve work_dir relative to project root
        root = get_project_root()
        cfg.work_dir = osp.join(str(root), 'models/YOLO-World/yolo_world/work_dirs',
                                osp.splitext(config["network2d"]["config_path"])[0].split("/")[-1])
        
        cfg.load_from = resolve_path(config["network2d"]["pretrained_path"])
    ...
    def inference_detector(self, images_batch):
        ...
        with torch.autocast(device_type='cuda', enabled=self.use_amp), torch.no_grad():
